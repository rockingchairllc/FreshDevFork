...
function getGeoCoords($address)
{
    ...
    $geoCodeURL = "http://maps.google.com/maps/api/geocode/json?address=".
                  urlencode(stripslashes($address))."&sensor=false";
    //echo "Nga=" . $geoCodeURL;   
...
function reverseGeoCode($lat,$lng)
{
    ...
    // call geoencoding api with param json for output
    $geoCodeURL = "http://maps.google.com/maps/api/geocode/json?oe=utf-8&address=$lat,$lng&sensor=false";
...
function getOSMGeoCoords($address)
{
    ...
    // output in JSON
    $geoCodeURL = "http://nominatim.openstreetmap.org/search?format=json&q=".
                  urlencode($address);
